...
- (id)initWithFrame:(CGRect)frame {
    ...
    if ((self = [super initWithFrame:frame])) {
		...
		gameController = [[BlockBlasterController alloc] init];
...
- (void)mainGameLoop {
    ...
    while(true) {
		...
		/* while( (time - lastTime)< CYCLE_TIME)
		 {
			 time = CFAbsoluteTimeGetCurrent();
		 }*/
		 
		float tm = (time - lastTime);
		float sleepTime = CYCLE_TIME - tm;
        ...
        if(_FPSCounter > 0.25f) {
            ...
            [_director setFramesPerSecond:_fps];
        }
