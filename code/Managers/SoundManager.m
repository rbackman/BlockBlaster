...
-(void)initSounds
{
	...
	{
		soundsLoaded=YES;
		[self setListenerPosition:CGPointMake(160, 0)];
		
		// Initialize the sound effects
			
		[self loadSoundWithKey:@"laser" fileName:@"laser.mp3"];
		[self loadSoundWithKey:@"missile" fileName:@"rocket.mp3"];
		[self loadSoundWithKey:@"explodeShort" fileName:@"Grenade.mp3"];
		[self loadSoundWithKey:@"explodeLong" fileName:@"ShipExplosion.mp3"];
		[self loadSoundWithKey:@"click" fileName:@"clickfast.mp3"];
		[self loadSoundWithKey:@"reward" fileName:@"CoinReward.mp3"];
		[self loadSoundWithKey:@"alienLaser" fileName:@"alienLaser.mp3"];
		[self loadSoundWithKey:@"turretLaser" fileName:@"TurretBlast.mp3"];
		[self loadSoundWithKey:@"launchGuardian" fileName:@"launchGuardian.mp3"];
		[self loadSoundWithKey:@"shieldActivate" fileName:@"shieldActivate.mp3"];
		[self loadSoundWithKey:@"shieldDown" fileName:@"shieldDown.mp3"];
		[self loadSoundWithKey:@"bonusStart" fileName:@"bonusStart.mp3"];
		[self loadSoundWithKey:@"bonusEnd" fileName:@"bonusEnd.mp3"];
		// Initialize the background music
	
		
		if(!self.isExternalAudioPlaying)
		{
			[self loadBackgroundMusicWithKey:@"music" fileName:@"oyshort.mp3"];
			[  self setMusicVolume:0.2 ];
			[self playMusicWithKey:@"music"  timesToRepeat:-1];

		}
		//[self loadBackgroundMusicWithKey:@"music" fileName:@"oy" fileExt:@"mp3"];
		//[self loadBackgroundMusicWithKey:@"music" fileName:@"ROB" fileExt:@"mp3"];
		
		
			
		// Set the master sound FX volume
		[self setFxVolume:0.5 ];	
	}
}
...
- (id)init {
	...
	if(self != nil) {
		...
		if (!isExternalAudioPlaying) {
			if(DEBUG)NSLog(@"INFO - SoundManager: No external audio playing so using the SoloAmbient audio session category");
			soundCategory = AVAudioSessionCategorySoloAmbient;
		} else {
			if(DEBUG)NSLog(@"INFO - SoundManager: External sound detected so using the Ambient audio session category");
...
- (void)loadSoundWithKey:(NSString*)aSoundKey fileName:(NSString*)aMusicFile {
...
- (void)loadBackgroundMusicWithKey:(NSString*)aMusicKey fileName:(NSString*)aMusicFile {
