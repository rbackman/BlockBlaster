...
- (void)calculateTexCoordsAtOffset:(CGPoint)aOffsetPoint subImageWidth:(int)aSubImageWidth 
	...
	if(aOffsetPoint.x != lastTextureOffset.x || aOffsetPoint.y != lastTextureOffset.y) {
		...
		tempTexCoords.tl_x = texWidthRatio * aOffsetPoint.x + dx;
		tempTexCoords.tl_y = texHeightRatio * aOffsetPoint.y + dy;
		tempTexCoords.tr_x = texWidthRatio * aSubImageWidth + (texWidthRatio * aOffsetPoint.x)+dx;
		tempTexCoords.tr_y = texHeightRatio * aOffsetPoint.y+dy;
		tempTexCoords.bl_x = texWidthRatio * aOffsetPoint.x+dx;
		tempTexCoords.bl_y = texHeightRatio * aSubImageHeight + (texHeightRatio * aOffsetPoint.y) + dy;
		tempTexCoords.br_x = texWidthRatio * aSubImageWidth + (texWidthRatio * aOffsetPoint.x)+dx;
		tempTexCoords.br_y = texHeightRatio * aSubImageHeight + (texHeightRatio * aOffsetPoint.y)+dy;
		
	//	printf("top right %g %g \n",tempTexCoords.tr_x,tempTexCoords.tr_y);
...
- (void)calculateVerticesAtPoint:(CGPoint)aPoint subImageWidth:(GLuint)aSubImageWidth subImageHeight:(GLuint)aSubImageHeight centerOfImage:(bool)aCenter {
	...
	double quadWidth = aSubImageWidth * scale  * scaleV.x;
	double quadHeight = aSubImageHeight * scale * scaleV.y;
	...
	if(aCenter) {
		vertices[0].bl_x = aPoint.x + -quadWidth / 2 - pivot.x;
		vertices[0].bl_y = aPoint.y + -quadHeight / 2 - pivot.y;
		
		vertices[0].br_x = aPoint.x + quadWidth / 2 - pivot.x;
		vertices[0].br_y = aPoint.y + -quadHeight / 2 - pivot.y;

		vertices[0].tl_x = aPoint.x + -quadWidth / 2 - pivot.x;
		vertices[0].tl_y = aPoint.y + quadHeight / 2 - pivot.y ;

		vertices[0].tr_x = aPoint.x + quadWidth / 2 - pivot.x;
		vertices[0].tr_y = aPoint.y + quadHeight / 2 - pivot.y;
	} else {
		vertices[0].bl_x = vertices[0].br_x = vertices[0].tl_x = vertices[0].tr_x = aPoint.x - pivot.x;
		vertices[0].bl_y = aPoint.y - pivot.y;
		vertices[0].bl_y = vertices[0].br_y = vertices[0].tl_y = vertices[0].tr_y = aPoint.y - pivot.y;
...
-(void)drawBoundingBox
{
	...
	Quad2f q = vertices[0];
	Vector2f ps =  Vector2fMake((q.bl_x+q.br_x+q.tl_x+q.tr_x)/4.0f,(q.bl_y+q.br_y+q.tl_y+q.tr_y)/4.0f);

	bounds = makeRec(ps, 32, 32, 0, 0);
	drawRec(bounds);
}
...
-(void)rotate:(int)deg
{
	rotation = DEGREES_TO_RADIANS(deg);
}
