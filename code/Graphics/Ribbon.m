...
//@synthesize color;
//@synthesize textureLength;
@synthesize stop;
@synthesize ribWidth;

+(id)ribbonWithWidth:(float)w  length:(float)l color:(Color4f)color fade:(float)fade
{
	self = [[[Ribbon alloc] initWithWidth:w  length:l color:color fade:fade] autorelease];
	return self;
}
...
-(void)update:(float)delta
{
		...
		if(!stop)
		{
			if(_parent)
			{
			if(![_parent active] || ![_parent alive] )stop=YES;
			}
		}
...
-(void)addPoint
{
	...
	{
		endDelay--;
		if(endDelay <= 0) alive = NO;	
	}
	else
	{
		Vector2f np = [_parent getTailPos];
		
		if((ABS(np.x-lastPoint.x)+ABS(np.y-lastPoint.y))>min)
		{	
			lastPoint = np;
			[self addPointAt:Vector2fToCGPoint([_parent getTailPos]) width:ribWidth];
		}
...
{
	GLubyte r = (GLubyte)color.red;
	GLubyte g = (GLubyte)color.green;
	GLubyte b = (GLubyte)color.blue;
	GLubyte a = (GLubyte)color.alpha;
	...
	if (begin < 50)
	{
		...
		//	glEnableClientState(GL_COLOR_ARRAY);
			...
			for (; i < end; ++i)
			{
				...
				{
					GLubyte bt = (GLubyte)(255.f - (alive * 255.f));
					colors[idx+3] = bt*[[Director sharedDirector] globalAlpha];
